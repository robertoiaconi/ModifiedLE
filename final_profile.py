...
### Get gradient of modified Lane-Emden pressure profile using the equations of hydrostatic equilibrium###
def MLE_dpdr(mle_r, mle_rho, mle_mass, mc):
    chi = np.array([mle_params.kernel.kernel(r)[1] for r in mle_r])
    gc = chi * cgs_G * mc * mle_r / mle_params.kernel.h**3
    mle_dpdr = -cgs_G * mle_mass * mle_rho / mle_r**2 - mle_rho * gc
    return mle_dpdr
...
mle_profile.dpdr = MLE_dpdr(mle_profile.r, mle_profile.rho, mle_profile.m, mle_params.mc)
full_profile.dpdr = np.concatenate((mle_profile.dpdr, cut_profile.dpdr))

full_profile.metalfrac = np.concatenate((mle_profile.metalfrac, cut_profile.metalfrac))

full_profile.hydrogenfrac = np.concatenate((mle_profile.hydrogenfrac, cut_profile.hydrogenfrac))

full_profile.metalweig = np.concatenate((mle_profile.metalweig, cut_profile.metalweig))

full_profile.metalchar = np.concatenate((mle_profile.metalchar, mle_profile.metalchar))

full_profile.massfrac = np.concatenate((mle_profile.massfrac, mle_profile.massfrac))

#mesa from interpolated T
full_profile_temp_init = s.temperature[-1]
full_profile.eint, full_profile.temp = MESA_eos(full_profile.metalfrac, full_profile.hydrogenfrac, full_profile.metalweig, full_profile.metalchar, mesa_eos_params.species, mesa_eos_params.chem_id, mesa_eos_params.net_iso, full_profile.massfrac, full_profile.rho, full_profile.p, full_profile_temp_init, 'Full profile')
...
#axes[1].plot(full_profile.r, full_profile.eint_ideal, linestyle = ':')
